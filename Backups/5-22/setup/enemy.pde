class Monster{ //class for the monsters in the game
  ...
  void display() { //useless invisible box
    rectMode(CORNER);
    noStroke();
    fill(0,255,255,0);
  ...
  void hpDisplay() { //displays an HP bar above the monster
    if(type == "Goblin"){
      rectMode(CORNERS);
      stroke(0);
      fill(200,0,0);
  ...
  void patrol(float pX, float pY, float pW, float pH) { //a patrol function. inputs are x,y,width,height. makes the goblin walk back and forth
  ...
  void halt(){ //stops the goblin
    haltX = xPos;
    haltY = yPos;
  }//end halt
  
  
  void collide (){ //monster collision detection
  ...
  void autoAttack(){ //the monster auto attack, on a timer of 1 second
    tempX = xPos;
    tempY = yPos;
  ...
  void death (int i){ //when the monster dies, gives player 20xp
    ...
    void gifAssign(){ //assigns a gif based on the monster's motion
      //GOBLIN
        //IDLE/AFK
